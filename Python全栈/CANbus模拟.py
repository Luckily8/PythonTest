...
class CANBus:
    def __init__(self):
        ...
        self.arbitration_event = threading.Event()
    ...
    def start_arbitration(self):
        with self.bus_lock:
            if not self.arbitration_in_progress and self.current_senders:
                ...
                self.current_senders = []
                self.arbitration_in_progress = False
                self.arbitration_event.set()  # 设置事件，通知仲裁已完成
    ...
    def run(self):
        while self.running:
            ...
            if winner:
                ...
                # 仲裁尚未开始或没有胜出
                pass
            # print(f"!=节点 {self.node_id} 本次仲裁结束")
            self.bus.arbitration_event.wait(timeout=0.5)  # 等待仲裁完成，增加超时
            self.bus.arbitration_event.clear()  # 清除事件

    def send_message(self, msg_id):
        print(f"Ready:节点 {self.node_id} 正在发送消息 ID: {msg_id}")
        # 模拟发送时间
        time.sleep(0.2)
        print(f"Done:节点 {self.node_id} 完成消息 ID: {msg_id} 的发送\n")
